...
bool str_check(Unit unit[], char str[], int i, int sel) // name 0, pos 1
{
	if (i == SIZE)
		return false;
	if (unit[i].name == NULL)
		str_check(unit, str, i + 1, sel);
...
{
	if (str_check(unit, name, 0, 0) == true)
	{
		printf("\nSame Name!!\n\n");
		return;
	}
...
{
	if (str_check(unit, name, 0, 0) == false)
		return;
	if (unit[i].name == NULL)
...
{
	if (str_check(unit, position, 0, 1) == false)
		return;
	if (unit[i].name == NULL)
